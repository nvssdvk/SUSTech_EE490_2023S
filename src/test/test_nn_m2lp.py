...
def train(tr_set, dv_set, model, config, device):
    ...
    while epoch < n_epochs:
        epoch_loss = 0
        model.train()  # set my_model to training mode
        for x, y in tr_set:  # iterate through the dataloader
            ...
            # batch_loss = my_model.cal_loss(pred / np.pi * 2, y / np.pi * 2)  # compute loss

            batch_loss.backward()  # compute gradient (backpropagation)
            optimizer.step()  # update my_model with optimizer
        ...
        # After each epoch, test your my_model on the validation (development) set.
        dev_loss = dev(dv_set, model, device)
        if dev_loss < min_loss:
            # Save my_model if your my_model improved
            min_loss = dev_loss
            print(f'Saving my_model (epoch = {epoch + 1}, loss = {min_loss})')
            torch.save(model.state_dict(), config['model_path'])  # Save my_model to specified path
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
        ...
        if early_stop_cnt > config['early_stop']:
            # Stop training if your my_model stops improving for "config['early_stop']" epochs.
            break
...
def dev(dv_set, model, device):
    model.eval()  # set my_model to evalutation mode
    total_loss = 0
    for x, y in dv_set:  # iterate through the dataloader
        ...
        with torch.no_grad():  # disable gradient calculation
            pred = model(x)  # forward pass (compute output)
            batch_loss = model.cal_loss(pred, y)  # compute loss
            # batch_loss = my_model.cal_loss(pred / np.pi * 2, y / np.pi * 2)  #
...
if __name__ == "__main__":
    ...
    #                   first_dim=config['first_dim']).to(device)  # Construct my_model and move to device
    # model_loss, model_loss_record = train(tr_set, dv_set, my_model, config, device)
    # # # %%
    # plot_learning_curve(model_loss_record, title='m2lp my_model')
    # del my_model
    # my_model = M2LP(alpha=config['alpha'], input_dim=tr_set.dataset.dim, block_num=config['block_num'],
    #              first_dim=config['first_dim']).to(device)  # Construct my_model and move to device
    # ckpt = torch.load(config['model_path'], map_location='cpu')  # Load your best my_model
    # my_model.load_state_dict(ckpt)
    # plot_pred(dv_set, my_model, device)
    # # %%
    # preds = test(tt_set, my_model, device)  # predict COVID-19 cases with your my_model
