...
def train(tr_set, dv_set, model, config, device):
    ...
    while epoch < n_epochs:
        epoch_loss = 0
        model.train()  # set neural_newwork to training mode
        for x, y in tr_set:  # iterate through the dataloader
            ...
            # batch_loss = neural_newwork.cal_loss(pred / np.pi * 2, y / np.pi * 2)  # compute loss

            batch_loss.backward()  # compute gradient (backpropagation)
            optimizer.step()  # update neural_newwork with optimizer
        ...
        # After each epoch, test your neural_newwork on the validation (development) set.
        dev_loss = dev(dv_set, model, device)
        if dev_loss < min_loss:
            # Save neural_newwork if your neural_newwork improved
            min_loss = dev_loss
            print(f'Saving neural_newwork (epoch = {epoch + 1}, loss = {min_loss})')
            torch.save(model.state_dict(), config['model_path'])  # Save neural_newwork to specified path
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
        ...
        if early_stop_cnt > config['early_stop']:
            # Stop training if your neural_newwork stops improving for "config['early_stop']" epochs.
            break
...
def dev(dv_set, model, device):
    model.eval()  # set neural_newwork to evalutation mode
    total_loss = 0
    for x, y in dv_set:  # iterate through the dataloader
        ...
        with torch.no_grad():  # disable gradient calculation
            pred = model(x)  # forward pass (compute output)
            batch_loss = model.cal_loss(pred, y)  # compute loss
            # batch_loss = neural_newwork.cal_loss(pred / np.pi * 2, y / np.pi * 2)  #
...
if __name__ == "__main__":
    ...
    #                   first_dim=config['first_dim']).to(device)  # Construct neural_newwork and move to device
    # model_loss, model_loss_record = train(tr_set, dv_set, neural_newwork, config, device)
    # # # %%
    # plot_learning_curve(model_loss_record, title='m2lp neural_newwork')
    # del neural_newwork
    # neural_newwork = M2LP(alpha=config['alpha'], input_dim=tr_set.dataset.dim, block_num=config['block_num'],
    #              first_dim=config['first_dim']).to(device)  # Construct neural_newwork and move to device
    # ckpt = torch.load(config['model_path'], map_location='cpu')  # Load your best neural_newwork
    # neural_newwork.load_state_dict(ckpt)
    # plot_pred(dv_set, neural_newwork, device)
    # # %%
    # preds = test(tt_set, neural_newwork, device)  # predict COVID-19 cases with your neural_newwork
