...
def cal_element_pattern(qe, uv, bv, theta):
    ...
    for index in range(len(theta)):
        # temp_1 = np.exp(1j * k * vector_normalize(uv * bv[index]))
        temp_1 = np.exp(1j * k * (np.dot(uv, bv[index])))
        temp_2 = np.sign(np.cos(theta[index])) * (np.abs(np.cos(theta[index]))) ** qe
        element_pattern[index] = temp_1 * temp_2
    return element_pattern


def cal_excitation(qf, qe, uv, fv, bv, phase, theta):
    ...
    for index in range(len(theta)):
        ...
        # theta_f = np.arccos(np.dot(-fv, bv_sub) / (np.linalg.norm(fv) * np.linalg.norm(bv_sub)))
...
if __name__ == "__main__":
    ...
    bv = np.zeros([len(theta), 3])
    bv[:, 0] = np.sin(np.deg2rad(theta)) * np.cos(np.deg2rad(phi))
    bv[:, 1] = np.sin(np.deg2rad(theta)) * np.sin(np.deg2rad(phi))
    bv[:, 2] = np.cos(np.deg2rad(theta))
